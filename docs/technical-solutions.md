...
### 6.1 日报提交：ValidateWorkContent + AssessCompleteness 合并

日期：2026-03-05

原方案（v1）：两次串行 LLM 调用
- 第一次：判断是否为工作汇报（ValidateWorkContent，fastModel）
- 第二次：判断内容是否足够具体（AssessCompleteness，fastModel）

优化方案（v2）：合并为一次调用 ValidateAndAssess，一个 prompt 同时返回 valid + sufficient

效果对比（本地测试，fastModel=qwen3-max）：

| 场景 | v1 预估耗时 | v2 实测耗时 | 结果 |
|------|------------|------------|------|
| 闲聊输入（拒绝） | ~3-4s（2次调用） | 2.2s（1次调用） | 正确拒绝并引导 |
| "修了个bug"（追问） | ~3-4s（2次调用） | 2.9s（1次调用） | 正确追问具体内容 |
| 具体描述（通过→生成摘要） | ~3-4s（2次调用）+摘要 | 5.8s（1次调用+摘要流式） | 正确生成摘要 |

结论：合并后每次日报提交省 1-2s，三种场景判断准确率不变。

### 6.2 Data Asking：TableList 从 3 张表扩展到 6 张

日期：2026-03-05

原方案：TableList = [daily_entries, members, daily_summaries]
优化方案：TableList = [daily_entries, members, daily_summaries, teams, topics, topic_activities]

效果：
- "哪个topic持续时间最长" → 原方案无法查询（Agent 不知道 topics 表），优化后正确返回 "FootPrints"
- "每个团队有多少人" → 原方案无法查询，优化后正确查询 teams + members 关联

### 6.3 Session 服务：原始 HTTP → SDK + 并行保存

日期：2026-03-05

原方案：session.go 手写 HTTP 请求，ListMessages 无界并发，saveMessages 串行
优化方案：改用 SDK（CreateLLMSession/ListLLMSessionMessages 等），ListMessages 加 semaphore=10，saveMessages user+assistant 并行

效果：功能不变，代码量减少约 40 行，消除 goroutine 爆炸风险。
