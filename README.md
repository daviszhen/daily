...
│  service/session.go  MOI LLM Proxy 会话/消息 API      │
...
4. 查询前自动将"我"替换为用户真名，确保 Agent 能准确定位数据
...
### Session / Message
会话和消息通过 MOI LLM Proxy Session API 持久化，不存本地数据库：
- 每个聊天会话对应一个 MOI Session
- 消息（用户输入 + 助手回复）异步写入 MOI Message
- 前端加载历史消息时从 MOI 拉取
...
- 周报日期范围解析：prompt 中直接给出本周一的具体日期，避免 LLM 推算偏差
- 意图分类器区分疑问句（query）和陈述句（report）
- 模式验证：选了模式后用 ClassifyIntent（无历史）验证输入匹配性

### 日报提交流程
1. `ExtractWorkContent`：从多轮对话历史中提取完整工作内容
2. `ValidateWorkContent`：验证是否为有效工作内容
3. `AssessCompleteness`：充分性检查，不足则返回追问
4. `StreamSummarize`：流式生成摘要
5. `DetectRisks`：检测风险项
6. 前端展示确认卡片 → 用户确认 → `POST /api/chat` with `action=confirm`
7. 同一人同一天多次提交时，`MergeDailySummary` 合并摘要
...
|------|------|------|
| `token` | `{token: "..."}` | 流式文本输出 |
| `result` | `{type, summary, risks, ...}` | 日报摘要确认卡片 |
| `thinking` | `{text: "..."}` | Data Asking 思考步骤 |
| `meta` | `{downloadTitle, downloadUrl}` | 周报下载信息 |
...
├── server/                     后端
│   ├── cmd/
│   │   ├── server/main.go      入口 + embed 前端
│   │   ├── catalog_init/       独立工具：初始化 Catalog 表结构
