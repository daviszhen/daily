...
export function DailyFeed(): React.ReactElement {
  ...
  const DateRangeBar = () => (
    <div className="flex items-center space-x-2 text-sm flex-wrap gap-y-2">
      ...
      <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{startInput} ~ {endInput}</span>
    </div>
  );
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold tracking-tight" style={{ color: 'var(--text-primary)' }}>团队动态</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>查看团队进度与成员信息。</p>
      </div>

      <div className="flex items-center justify-between mb-6 border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="flex space-x-1">
          {([['by-member', BarChart2, '按成员'], ['by-topic', Hash, '按 Topic'], ['members', Users, '成员管理']] as const).map(([key, Icon, label]) => (
            <button key={key} onClick={() => setTab(key as Tab)}
              ...
              style={tab === key ? { borderColor: 'var(--btn-primary)', color: 'var(--text-primary)' } : { borderColor: 'transparent', color: 'var(--text-secondary)' }}>
      ...
      {tab === 'by-member' && (
        <div className="space-y-4">
          ...
          {loading ? <LoadingSkeleton /> : memberFeeds.length === 0 ? (
            ...
            <div key={mf.member_id} className="rounded-xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              <div className="px-4 py-3 font-medium text-sm" style={{ background: 'var(--bg-sidebar)', color: 'var(--text-primary)' }}>
                {mf.member_name}
                <span className="ml-2 text-xs font-normal" style={{ color: 'var(--text-secondary)' }}>{mf.items.length} 条记录</span>
              </div>
                  ...
                  <div key={i} className="px-4 py-3 text-sm">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <span className="text-xs font-mono mr-2" style={{ color: 'var(--text-muted)' }}>{item.daily_date?.slice(0, 10)}</span>
      ...
      {tab === 'by-topic' && (
        <div className="space-y-4">
          ...
          {loading ? <LoadingSkeleton /> : topicFeeds.length === 0 ? (
            ...
            <div key={tf.topic} className="rounded-xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              <div className="px-4 py-3 flex items-center justify-between" style={{ background: 'var(--bg-sidebar)' }}>
                <div>
                  <span className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>{tf.topic}</span>
                  <span className="ml-2 text-xs" style={{ color: 'var(--text-secondary)' }}>{tf.items.length} 条活动</span>
                </div>
                <div className="flex space-x-1">
                  {tf.members.map(name => (
                    <span key={name} className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--bg-active)', color: 'var(--text-dim)' }}>{name}</span>
                  ))}
                </div>
              </div>
              <div className="divide-y" style={{ borderColor: 'var(--border-light)' }}>
                {tf.items.map((item, i) => (
                  <div key={i} className="px-4 py-3 text-sm">
                    <span className="text-xs font-mono mr-2" style={{ color: 'var(--text-muted)' }}>{item.daily_date?.slice(0, 10)}</span>
                    <span className="text-xs mr-2 font-medium" style={{ color: 'var(--text-dim)' }}>{item.member_name}</span>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: 'var(--bg-sidebar)', borderBottom: '1px solid var(--border)' }}>
                    <th className="text-left px-4 py-3 font-medium" style={{ color: 'var(--text-dim)' }}>姓名</th>
                    <th className="text-left px-4 py-3 font-medium" style={{ color: 'var(--text-dim)' }}>职位</th>
                    <th className="text-left px-4 py-3 font-medium" style={{ color: 'var(--text-dim)' }}>团队</th>
                    <th className="text-left px-4 py-3 font-medium" style={{ color: 'var(--text-dim)' }}>状态</th>
                    ...
                    return (
                      <tr key={m.id} className="transition-colors" style={{ cursor: 'default' }}
                        ...
                        <td className="px-4 py-3">
                          {isEditing ? (
                            <select className="rounded px-2 py-1 text-sm focus:outline-none" style={{ border: '1px solid var(--border)' }}
                        ...
                        <td className="px-4 py-3">
                          ...
                          ) : <span style={{ color: 'var(--text-secondary)' }}>{m.team_name || '—'}</span>}
                        </td>
                        <td className="px-4 py-3">
                          {isEditing ? (
                            <select className="rounded px-2 py-1 text-sm focus:outline-none" style={{ border: '1px solid var(--border)' }}
                        ...
                        <td className="px-4 py-3 text-right">
                          {isEditing ? (
                            <div className="flex items-center justify-end space-x-2">
                              ...
                              <button onClick={cancelEdit} className="p-1 transition-colors" style={{ color: 'var(--text-muted)' }} title="取消"><X size={16} /></button>
                            </div>
                          ) : (
                            <div className="flex items-center justify-end space-x-1">
                              <button onClick={() => startEdit(m)} className="p-1 transition-colors" style={{ color: 'var(--text-muted)' }} title="编辑"><Pencil size={15} /></button>
                              <button onClick={() => setDeleteTarget(m)} className="p-1 transition-colors hover:text-red-500" style={{ color: 'var(--text-muted)' }} title="删除"><Trash2 size={15} /></button>
                            </div>
                          )}
                        </td>
                      </tr>
...
function LoadingSkeleton() {
  return <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="animate-pulse h-14 rounded-lg" style={{ background: 'var(--bg-accent)' }} />)}</div>;
}

function Empty({ text }: { text: string }) {
  return <div className="text-sm text-center py-16" style={{ color: 'var(--text-muted)' }}>{text}</div>;
}
